...
**解决：** 默认用浏览器直接打开短链，等 JS 重定向完成后立刻执行 `references/account-safety.md` 的账号安全门禁。只有确认未登录后，才能从 tab URL 获取最终地址，并把该最终地址交给 `yt-dlp` 优先提取。

## 2. 禁止使用登录账号

本 skill 不使用任何小红书登录账号。使用登录账号进行抓取有账号处罚风险。

**处理：**
- 检测到已登录小红书时，立即停止浏览器提取。
- 提示用户退出登录或切换到未登录隔离浏览器环境。
- 未登录导致评论、互动数、视频源或图片源不可用时，标注原因并继续；不要要求用户登录。
